...
def run(context: FessContext) -> None:
    ...
    try:
        ...
        cleanup = Cleanup()
        with cleanup.guard(f"role '{xss_marker}'"):
            delete_by_name(context, page, "/admin/role/", xss_marker)
        cleanup.escalate()
    ...
    try:
        ...
        cleanup = Cleanup()
        with cleanup.guard(f"role '{duplicate_role}'"):
            delete_by_name(context, page, "/admin/role/", duplicate_role)
        cleanup.escalate()
